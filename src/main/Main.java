...
public class Main {
	...
	public static void main(String[] args) {
		...
		if(algoritmo.compareTo("bubble_sort")==0) {
			...
			long ram=0;long times=0;
			for (int i = 0; i < 5; i++) {
				long[] temp = runningQuick(num_ramdom, name_file_out);
				times = times + temp[0];
				ram = ram+ temp[1];
			}
			long media_ram_final = ram/5;//em bytes
			...
			long[] bestCase = runningQuick(num_melhor_caso, name_file_out);//calcula uma vez pro melhor caso. Array ja ordenado
			long[] worstCase = runningQuick(num_pior_caso, name_file_out);//calcula uma vez pro pior caso também
			
			System.out.println("=============================================================================");
			System.out.println("		RESULTADOS QUICK SORT (N="+num_melhor_caso.length+")				");
			System.out.println("=============================================================================");
			//System.out.println("");
			...
			long ram=0;long times=0;
			for (int i = 0; i < 5; i++) {
				long[] temp = runningShell(num_ramdom, name_file_out);
				times = times + temp[0];
				ram = ram+ temp[1];
			}
			long media_ram_final = ram/5;//em bytes
			...
			long[] bestCase = runningShell(num_melhor_caso, name_file_out);//calcula uma vez pro melhor caso. Array ja ordenado
			long[] worstCase = runningShell(num_pior_caso, name_file_out);//calcula uma vez pro pior caso também
			
			System.out.println("=============================================================================");
			System.out.println("		RESULTADOS SHELL SORT (N="+num_melhor_caso.length+")				");
			System.out.println("=============================================================================");
			//System.out.println("");
	...
	public static long[] runningQuick(int[] vetor, String name_file_out) {
		//QuickSort
		System.out.print("\n-------------------------\n");
		//System.out.println("Quick");
		long timeQSInicial = System.currentTimeMillis();//inicia o time
		GetFormat quick  = Algorithms.quickSort(vetor, vetor[0], vetor.length-1);
		...
		saveToFile(name_file_out, "quick_sort", quick.getArrayOrdenado().length, Casos.getCaso(), tq, quick.getConsumoRAM());
		long[] result = new long[2];
		result[0] = tq; result[1]= ram;
		return result;
	}
	public static long[] runningShell(int[] vetor, String name_file_out) {
		//ShellSort
		System.out.println("\n--------------------------------------------------------------------------------------------------------");
		System.out.println("Shell ");
		long timeSSInicial = System.currentTimeMillis();//inicia o time
		GetFormat shell = Algorithms.shellSort(vetor);
		...
		saveToFile(name_file_out, "shell_sort", shell.getArrayOrdenado().length, Casos.getCaso(), ts, shell.getConsumoRAM());

		
		long[] result = new long[2];
		result[0] = ts; result[1]= ram;
		return result;
